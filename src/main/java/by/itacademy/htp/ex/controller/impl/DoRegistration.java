	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (userDataValidationService.checkRegistrationForm(login, password, name, surname, email)) 
			{
				...
				request.getSession(true).setAttribute(ConstName.ATTRIBUTE_REGISTRATION_PASSED, "registered");
		...
		
		{
			 ...
			 request.getSession().setAttribute(ConstName.ATTRIBUTE_FOR_MESSAGE_ERROR_PAGE, "empty fields or wrong data, check");
			 response.sendRedirect("controller?command=go_to_erro_page");
